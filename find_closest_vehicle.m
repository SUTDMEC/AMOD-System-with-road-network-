function [closest_vehicle_position] = find_closest_vehicle(vehicle_availability_map, possible_movements, present_location, heuristic,time)
...
type = 'A';
C = vehicle_availability_map(present_location(1), present_location(2),:,time); 
A = [C(1),C(2)];
is_type_in_A = any(strfind(A,type));
...
count = 0;
    while  not(found) && not(resign)
        ...
        if (~size(searched_stack,1))
            ...
            C = vehicle_availability_map(x, y,:,time); 
            A = [C(1),C(2)];
            is_type_in_A = any(strfind(A,type));
                ...
                for iii= 1: size (possible_movements,1)
                    ...
                    if (y2> 0 && y2 < size(vehicle_availability_map,1)+1 && x2>0 && x2 < size(vehicle_availability_map,2)+1)
                        if (checked(y2, x2) == 0 && size(vehicle_availability_map(y2,x2,:,time),3)> -1)
                            ...
                            C = vehicle_availability_map(x2,y2,:,time); 
                            A = [C(1),C(2)];
                            is_type_in_A = any(strfind(A,type));
...
if(object_find_check)
    closest_vehicle_position = closest_object_found(1,2:3); %[column,row]
end
